...
use hpc_algorithms::{
    SPlusTree32Layout, SPlusTreeLayout, STreeLayout, s_plus_tree_search_neon,
...
type SPlus32Fn = fn(&SPlusTree32Layout, i32) -> Option<usize>;
...
fn bench_variant_splus32(c: &mut Criterion, name: &str, func: SPlus32Fn) {
    let mut group = c.benchmark_group(name);
    for &(label, len) in INPUT_SIZES {
        let values = make_sorted_values(len);
...
fn bench_s_tree(c: &mut Criterion) {
    ...
    bench_variant_splus32(c, "s_plus_tree32_scalar", s_plus_tree32_search_scalar);
    bench_variant_splus32(c, "s_plus_tree32_neon", s_plus_tree32_search_neon);
}
